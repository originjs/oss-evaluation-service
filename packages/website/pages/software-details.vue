        ...
        <div float-left w-825px>
          <div position-relative flex flex-items-center>
            <el-tooltip effect="light" :teleported="false">
              <div mt--5px mr-12px max-w-600px font-size-7 font-bold line-height-normal class="text-over">{{ baseInfo.name }}</div>
              <template #content>
          ...
          <el-tag v-for="(label, idx) in baseInfo.labels" :key="idx" :type="tagType(idx)" mr-2 mb-2>{{ label }}</el-tag>
        </div>
    ...
    <div w-1280px m-auto>
      ...
      <el-card>
        ...
        <div flex flex-items-center h-30px>
          ...
          <el-progress :percentage="performance.benchmarkScore" text-inside :stroke-width="15" flex-auto ml-6 mr-6 />
      ...
      <el-card>
        ...
        <div h-207px flex flex-wrap justify-between content-between>
          <div position-relative pt-3 pd-3 pl-4 pr-4 w-607px h-92px bg-coolgray-50>
            ...
            <div class="position-absolute right-18px top-50% w-30px h-30px border-rd-50% bg-blue text-center translate-y--50%">
...
const baseInfo = ref({
  ...
  name: 'vuejs/vue ',
  introduction: 'Vue.js is a progressive,incrementally-adoptable JavaScript framework for building UI on the web.',
...
const docItems = ref([
  {
    title: 'Changelog',
    content: 'The npm package reactives a total of 22910623 downloads a week.As such,we scored react popularity level to be Key ecosystem project.',
  },
  {
    title: 'Governance',
    content: 'The npm package reactives a total of 22910623 downloads a week.As such,we scored react popularity level to be Key ecosystem project.',
