...
export async function searchProject(
  ...
  const searchSql = `
      ...
      from view_projects projects
      where projects.full_name like concat('%', :keyword, '%')
          ${techStack ? 'and projects.p_id in (select distinct p_id from project_tech_stack where subcategory = :techStack)' : ''}
...
export async function getRadarList(): Promise<TechRadarItem[]> {
  const sql = `select p_id                                               as id,
                      name                                               as label,
                      radar_quadrant                                     as quadrant,
                      radar_ring                                         as ring,
                      radar_moved                                        as moved,
                      CONCAT('/#/software-details?repoName=', full_name) as link
               from project_tech_stack
               where radar_ring is not null`;
