...
export async function getAlternativeProjects(fullName: string): Promise<AlternativeInfo[]> {
  ...
  const querySimilarByAiSql = `SELECT alternative_id,
                                      alternative_name,
                                      alternative_url,
                                      source,
                                      owner_avatar_url,
                               ...
                               WHERE a.full_name = :fullName
                                 AND approved = 1
                               ORDER BY distance, stargazers_count DESC
                               LIMIT ${ALTERNATIVE_SIZE}`;
  // similar project by category and subcategory
  const querySimilarByCategorySql = `SELECT p.p_id,
                p.full_name,
                p.html_url,
  ...
  const [similarByAi, similarByCategory] = await Promise.all([
    sequelize.query(querySimilarByAiSql, {
      replacements: { fullName },
      type: sequelize.QueryTypes.SELECT,
    }),
    sequelize.query(querySimilarByCategorySql, {
      replacements: { repoName: `https://github.com/${fullName}` },
      type: sequelize.QueryTypes.SELECT,
    }),
  ]);

  const alternatives = similarByAi.map(item => ({
  ...
  for (const item of similarByCategory) {
    // exclude it self
